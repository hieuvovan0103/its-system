  ...
  const onSubmit = async (data: AssessmentFormData) => {
    try {
      if (editingAssessment) {
        const updated = await assessmentService.update(editingAssessment.id, data);
        setAssessments(assessments.map((a) => (a.id === editingAssessment.id ? updated : a)));
      } else {
        const created = await assessmentService.create(data);
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-5 text-sm">
                      <span className="flex items-center gap-1.5 text-gray-500">
                        ...
                        <span className="font-medium">{assessment.totalScore}</span> points
                      </span>
                      <span className="flex items-center gap-1.5 text-gray-500">
                        <ClipboardList className="w-4 h-4 text-blue-500" />
                        <span className="font-medium">{assessment.questions.length}</span> questions
                      </span>
                      {daysUntilDue !== null && (
